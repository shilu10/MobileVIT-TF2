...
class MobileVIT(tf.keras.models.Model):
  def __init__(self, model_variant, classifier_dropout, num_classes, *args, **kwargs):
    ...
    self.layer_1, out_channels = self._make_layer(
            input_channel=in_channels, config=mobilevit_config["layer1"]
        )
    ...
    self.layer_2, out_channels = self._make_layer(
            input_channel=in_channels, config=mobilevit_config["layer2"]
        )
    ...
    self.layer_3, out_channels = self._make_layer(
            input_channel=in_channels, config=mobilevit_config["layer3"]
        )
    ...
    self.layer_4, out_channels = self._make_layer(
            input_channel=in_channels, config=mobilevit_config["layer4"]
        )
    ...
    self.layer_5, out_channels = self._make_layer(
            input_channel=in_channels, config=mobilevit_config["layer5"]
        )
  ...
  def _make_layer(self, input_channel, config):
    # block_type
    block_type = config.get('block_type')

    if block_type.lower() == "mv2":  # mobilenetv2 block 
      return self.build_mv2_block(input_channel, config)

    else:
      return self.build_mobilevit_block(input_channel, config)

  def build_mobilevit_block(self, input_channels, config):
    block = []
    ...
    print(head_dim, transformer_dim, ffn_dim)
    if head_dim is None:
    ...
    return tf.keras.Sequential(block), input_channels 

  def build_mv2_block(self, input_channels, config):
